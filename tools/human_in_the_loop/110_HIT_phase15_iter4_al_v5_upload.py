"""
Phase 15 — iteration 4 launcher (al_v5 held-out val).

Uploads predictions from the al_v5 best_model (epoch 5, F5=0.9569,
R=0.97, P=0.78) for review of the 7 held-out val datasets:
  iseb05_19012023, flpr02_28012023, pwc03_med_11122021,
  mbbe03_med_09122021, iswc01_02_19012023, srecc02_12022023,
  ispa05_15122021.

Inference run with `adapt_ts=0.20` + `score_threshold=0.30` —
matches the training-time eval recipe so the displayed predictions
correspond to the same model behaviour.
"""
...
AL_V5_ROOT = Path(
    "/home/christian/data/training_data/2026_06_02_al_v5_canonical/al_v5_2026_06_02"
)
VAL_IMG_DIR = AL_V5_ROOT / "val/Default"
VAL_GT_CSV = AL_V5_ROOT / "val/herdnet_format.csv"

DET_CSV: Path | None = None  # auto-pick newest al_v5_inference_e5_*
...
def _find_det_csv() -> Path:
    ...
    candidates = sorted(
        Path("/home/christian/hnee/HerdNet/output").glob(
            "al_v5_inference_e5_*/detections.csv"
        )
    )
    if not candidates:
        raise FileNotFoundError("No al_v5 detections.csv found.")
    chosen = candidates[-1]
...
def main() -> None:
    ...
    dataset_name = f"phase15_iter4_al_v5_{date.today().strftime('%Y_%m_%d')}"
    ...
    logger.info("=== Phase15 iter4 (al_v5) upload complete ===")
