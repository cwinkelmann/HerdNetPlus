"""
Phase 15 — iteration 1 download. Pulls the human-corrected annotations
from the CVAT task launched by ``104_HIT_phase15_iter1_promote_and_upload.py``
and applies them to the *current* Hasty master at
``/data/mnt/storage/Iguanas_From_Above/training_data/2026_05_06/2026_05_06_labels.json``
(which already contains the iter-0 corrections promoted on 2026-06-01).

The download uses the ``hasty_id`` round-trip (commit 2708eca) so kept
GT keypoints retain their original master IDs and edits land precisely.

Run after the human reviewer has worked through CVAT task 2297456.
Writes:
  - 2026_05_06_labels_phase15_iter1_corrected.json   (the updated master)
...
OUTPUT_HASTY = HASTY_MASTER.parent / f"{HASTY_MASTER.stem}_phase15_iter1_corrected.json"
EDIT_LOG = Path("/home/christian/hnee/HerdNet/data/phase15_edit_log.csv")
ITERATION_ID = "phase15_iter1_val_test"
