...
class BiFPN(nn.Module):
    """
    Bidirectional Feature Pyramid Network with texture-aware fusion.

    Improvements over basic FPN:
    - Top-down AND bottom-up pathways (bidirectional)
    - Learnable fusion weights per scale per direction
    - Texture-aware lateral connections
    - Bottom-up pass carries fine-grained texture info to coarser scales

    This helps camouflaged object detection by allowing texture signals
    from the finest scale (where Gabor features are fused) to propagate
    upward and inform context at coarser scales.
    """
    ...
    def __init__(
        ...
        # Learnable bottom-up fusion weights (one per connection)
        self.bu_weights = nn.Parameter(torch.ones(self.num_levels - 1) * 0.5)
    ...
    def forward(self, features: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        Returns:
            BiFPN features at multiple scales
        """
        ...
        # Apply texture attention to finest scale (before bottom-up)
        ...
        # This carries texture/Gabor signals from P2 upward to P3, P4, P5
        bu_w = torch.sigmoid(self.bu_weights)
        for i in range(self.num_levels - 1):
            downsampled = F.interpolate(outputs[i], size=outputs[i + 1].shape[2:], mode='bilinear', align_corners=False)
            outputs[i + 1] = outputs[i + 1] + bu_w[i] * self.bu_convs[i](downsampled)
...
class DeformableBranch(nn.Module):
    """A detection branch using deformable convolutions.

    Deformable conv learns to sample where features actually are,
    adapting to irregular object shapes (curled iguanas, partial occlusion).
    """

    def __init__(self, in_channels: int, hidden_channels: int, dilation: int = 1):
        super().__init__()
        from torchvision.ops import DeformConv2d

        # Offset prediction for deformable conv
        # 2 * 3 * 3 = 18 offsets (x,y for each position in 3x3 kernel)
        self.offset_conv = nn.Conv2d(
            in_channels, 2 * 3 * 3, kernel_size=3, padding=dilation, dilation=dilation, bias=True
        )
        nn.init.zeros_(self.offset_conv.weight)
        nn.init.zeros_(self.offset_conv.bias)

        self.deform_conv = DeformConv2d(
            in_channels, hidden_channels, kernel_size=3, padding=dilation, dilation=dilation, bias=False
        )
        self.bn1 = nn.BatchNorm2d(hidden_channels)
        self.act1 = nn.GELU()
        self.cbam = CBAM(hidden_channels, reduction=8)

        # Second conv (standard, not deformable — the offset already adapted sampling)
...
class CamouflageDetectionHead(nn.Module):
    """
    Detection head v2 with deformable convolutions for camouflaged objects.

    Improvements over v1:
    - Deformable convolutions adapt sampling to irregular object shapes
    - Edge-enhanced processing for subtle boundaries
    - Multi-scale branches (d=1, d=2, d=4) with CBAM attention
    ...
    def __init__(
        ...
        # Deformable multi-scale branches
        self.fine_branch = DeformableBranch(in_channels, hidden_channels, dilation=1)
        self.medium_branch = DeformableBranch(in_channels, hidden_channels, dilation=2)
        self.coarse_branch = DeformableBranch(in_channels, hidden_channels, dilation=4)
    ...
    def _init_weights(self):
        ...
        # Re-zero deformable offsets (they get kaiming_normal from the loop above)
        for branch in [self.fine_branch, self.medium_branch, self.coarse_branch]:
            nn.init.zeros_(branch.offset_conv.weight)
            nn.init.zeros_(branch.offset_conv.bias)

    def forward(self, x: torch.Tensor, target_size: Optional[Tuple[int, int]] = None) -> torch.Tensor:
        """
        Args:
            x: Input features [B, in_channels, H, W]
            target_size: Optional target output size (H, W).
        ...
        # Multi-scale deformable processing
...
# =============================================================================
# Main Model - CamouflageHerdNetConvNeXtV3
# =============================================================================

@MODELS.register()
class CamouflageHerdNetConvNeXtV3(nn.Module):
    """
    ...
    def __init__(
        ...
        if debug:
            logger.info(f"\nInitializing CamouflageHerdNetConvNeXtV3:")
        ...
        # ConvNeXt-V2 backbone configurations (FCMAE pre-training, improved over V1)
        # Same channel dims as V1 — drop-in replacement with better features
        convnext_models = {
        ...
        # Auxiliary heatmap heads on P3 and P4 for deep supervision
        # These produce heatmaps at coarser resolutions, supervised with downsampled FIDT GT
        # Only used during training — removed at inference via self.training check
        self.aux_head_p3 = nn.Conv2d(fpn_channels, 1, kernel_size=1)
        self.aux_head_p4 = nn.Conv2d(fpn_channels, 1, kernel_size=1)
        nn.init.normal_(self.aux_head_p3.weight, std=0.01)
        nn.init.constant_(self.aux_head_p3.bias, -2.0)
        nn.init.normal_(self.aux_head_p4.weight, std=0.01)
        nn.init.constant_(self.aux_head_p4.bias, -2.0)

        # P2P auxiliary head — predicts point locations directly from FPN features
        # Uses reference grid + offset regression, supervised by HungarianLoss
        self.p2p_num_queries = 100
        p2p_hidden = 256
        fpn_ch = fpn_channels  # 256

        # Reference grid: fixed positions covering the normalized image
        grid_size = int(self.p2p_num_queries ** 0.5)  # 10x10
        ys = torch.linspace(0.05, 0.95, grid_size)
        xs = torch.linspace(0.05, 0.95, grid_size)
        grid = torch.stack(torch.meshgrid(xs, ys, indexing='xy'), dim=-1).reshape(-1, 2)
        self.register_buffer('p2p_reference_grid', grid)  # [100, 2]

        # Position embedding for each query
        self.p2p_pos_embed = nn.Embedding(self.p2p_num_queries, p2p_hidden)

        # Feature projection (FPN channel -> hidden)
        self.p2p_input_proj = nn.Linear(fpn_ch, p2p_hidden)

        # Classification head: is there an object at this query?
        self.p2p_cls_head = nn.Sequential(
            nn.Linear(p2p_hidden * 2, p2p_hidden),
            nn.ReLU(),
            nn.Linear(p2p_hidden, num_classes)
        )

        # Coordinate head: offset from reference point
        self.p2p_coord_head = nn.Sequential(
            nn.Linear(p2p_hidden * 2, p2p_hidden),
            nn.ReLU(),
            nn.Linear(p2p_hidden, 2)
        )
    ...
    def forward(self, x: torch.Tensor, debug: bool = False) -> Tuple[torch.Tensor, torch.Tensor] | Dict:
        ...
        # During training, return auxiliary outputs for deep supervision + P2P
        if self.training:
            # Auxiliary heatmaps (upsampled to match main resolution)
            aux_p3_raw = self.aux_head_p3(fpn_features[1]) / self.temperature.clamp(min=0.1)
            aux_p4_raw = self.aux_head_p4(fpn_features[2]) / self.temperature.clamp(min=0.1)
            aux_p3 = torch.sigmoid(F.interpolate(aux_p3_raw, size=heatmap.shape[2:], mode='bilinear', align_corners=False))
            aux_p4 = torch.sigmoid(F.interpolate(aux_p4_raw, size=heatmap.shape[2:], mode='bilinear', align_corners=False))

            # P2P auxiliary head: sample FPN features at reference grid, predict logits + offsets
            feat_p2 = fpn_features[0]  # [B, 256, H/4, W/4] — finest FPN scale
            B = feat_p2.shape[0]

            # Sample features at reference grid positions using grid_sample
            ref = self.p2p_reference_grid.unsqueeze(0).expand(B, -1, -1)  # [B, 100, 2]
            # grid_sample expects grid in [-1, 1] range
            grid = (ref * 2 - 1).unsqueeze(2)  # [B, 100, 1, 2]
            sampled = F.grid_sample(feat_p2, grid, mode='bilinear', align_corners=True)  # [B, 256, 100, 1]
            sampled = sampled.squeeze(-1).permute(0, 2, 1)  # [B, 100, 256]

            # Project and combine with position embeddings
            pos_idx = torch.arange(self.p2p_num_queries, device=feat_p2.device)
            pos_emb = self.p2p_pos_embed(pos_idx).unsqueeze(0).expand(B, -1, -1)  # [B, 100, 256]
            proj = self.p2p_input_proj(sampled)  # [B, 100, 256]
            combined = torch.cat([proj, pos_emb], dim=-1)  # [B, 100, 512]

            # Predict
            p2p_logits = self.p2p_cls_head(combined)  # [B, 100, num_classes]
            p2p_offsets = torch.tanh(self.p2p_coord_head(combined)) * 0.5  # [B, 100, 2]
            p2p_points_norm = (ref + p2p_offsets).clamp(0, 1)  # [B, 100, 2]

            # Pack P2P outputs for the HungarianLoss
            p2p_out = {
                'logits': p2p_logits,
                'pred_points_normalized': p2p_points_norm,
                'reference_points': ref,
                'offsets': p2p_offsets,
                'image_size': (original_h, original_w),
            }

            # Store P2P output for external loss computation
            # The P2PLossAdapter in the loss config will access this via the model
            self._p2p_output = p2p_out

            return heatmap, cls_out, aux_p3, aux_p4
...
def create_camouflage_herdnet_convnext(
...
) -> CamouflageHerdNetConvNeXtV3:
    """
    Create CamouflageHerdNetConvNeXtV3 model optimized for camouflaged objects.
    ...
    Returns:
        CamouflageHerdNetConvNeXtV3 model
    """
    model = CamouflageHerdNetConvNeXtV3(
