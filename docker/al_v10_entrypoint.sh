#!/bin/bash
# al_v10 single training run (post phase-15 iter-7, H-aware loader).
#
# Trains B4 ConvNeXtV2 (tiny) with hard_negative_probability=0.25 /
# empty_probability=0.10 on the al_v10 train split (5,991 imgs), val on
# 646 untouched ortho images across 5 islands (Marchena, Floreana,
# Fernandina, Isabela, Pinta). Warm-start from phase8 b4_seed42.
...
echo "============================================================"
echo "  al_v10 training — $(date '+%Y-%m-%d %H:%M:%S')"
echo "  SEED:                ${SEED}"
...
echo "  HARD_NEG_PROB:       ${HARD_NEG_PROB}"
...
OUT_DIR=/app/output/al_v10_b4_seed42/$(date '+%Y-%m-%d')/$(date '+%H-%M-%S')
mkdir -p "$OUT_DIR"
...
python tools/train.py \
    ...
    +datasets.train.albu_transforms.ObjectAwareRandomCrop.hard_negative_probability="${HARD_NEG_PROB}" \
    ...
    wandb_run="${WANDB_RUN}" \
    +wandb_tags=[active_learning,v10,b4,balanced,f2_score,hnp25_ep10,post_iter7,ortho_val_5islands] \
